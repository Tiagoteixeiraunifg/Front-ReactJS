...
import { setId,  setNome,  setSobrenome,  setEmail,  setCpf,  setCelular,
   ...
   setSalvo as setSalvoR,  setNovo as setNovoR, setEditando as setEditandoR} from '../../redux/reducers/clientReducer'
...
const post = async () => {
  setLoading(true);
        apiAuth.post('/v1/clientes', client, { headers: {'Content-Type': 'application/json', 'Accept': '*/*',
...
const put = async (idClient: number) => {
  setLoading(true);
        apiAuth.put('/v1/clientes/'+idClient, client, { headers: {'Content-Type': 'application/json', 'Accept': '*/*',
        'Authorization': `Bearer ${userLogin.token}`}})
        .then(function (response) {
            setCliente(response.data.data);
            setFalha(false);
